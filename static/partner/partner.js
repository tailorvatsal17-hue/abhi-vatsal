document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.location.pathname === '/partner/dashboard') {
        ...
        fetch(`http://localhost:3000/api/partners/${partnerId}/bookings`, { headers })
            .then(res => res.json())
        ...
        const fetchPartnerAvailability = async () => {
            try {
                const response = await fetch('http://localhost:3000/api/partners/availability', {
        ...
        const deleteAvailability = async (id) => {
            try {
                const response = await fetch(`http://localhost:3000/api/partners/availability/${id}`, {
                    method: 'DELETE',
                    headers: { 'Authorization': `Bearer ${token}` }
                });
    ...
    if (window.location.pathname === '/partner/bookings') {
        ...
        const fetchPartnerBookings = () => {
            fetch(`http://localhost:3000/api/partners/${partnerId}/bookings`, { headers: { 'Authorization': `Bearer ${token}` } })
                .then(res => res.json())
                .then(data => {
