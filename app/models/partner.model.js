...
Partner.updateById = (id, partner, result) => {
    sql.query(
        "UPDATE partners SET name = ?, service_id = ?, description = ?, profile_image = ?, work_images = ?, pricing = ? WHERE id = ?",
        [partner.name, partner.service_id, partner.description, partner.profile_image, partner.work_images, partner.pricing, id],
        (err, res) => {
            if (err) {
                console.log("error: ", err);
                result(null, err);
